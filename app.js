...
// Middleware para processar as requisições e envio de JSON
...
app.post("/cadastro", (req, res) => {
  ...
  if ((!req.body || Object.keys(req.body), length === 0)) {
    ...
    return (
      res,
      status(400).json({ success: false, message: "Nenhum dado recebido." })
    );
  }

  console.log("Corppo da requisição:", JSON.stringify(req.body, null, 2));
  ...
  const checkUseQuery = "SELECT * FROM users WHERE username = ? OR email = ?";
  db.get(checkUseQuery, [username, email], (err, row) => {
    if (err) {
      console.error(
    ...
    if (row) {
      ...
      const insertQuery =
        "INSERT INTO users (username, password, email, celular, cpf, rg) VALUE (?, ?, ?, ?, ?, ?,";
      db.run(
        insertQuery,
        ...
        function (err) {
          if (err) {
            ...
            return res.status(500).json({
              success: false,
              message: "Erro interno do servidor aao cadastrar usuário.",
            });
          }
